...
| **輸出信號控制** | Hook/CI/工具輸出有沒有針對 AI 消費設計，盡量減少 context 噪音？ | 錯誤單行 + grep-friendly，log 寫檔案不打 console，輸出聚合統計而非原始資料 = A, 混合 = C, 冗長原始輸出打到 console = F |
...
| **資源護欄** | 有沒有機制防止 agent 在低價值活動上花費不成比例的時間？ | 有測試採樣/timeout/迭代限制 = A, 有部分上限 = C, agent 可以無限迴圈 = F |
...
| **E2E 驗證閘門** | 系統有沒有要求端到端功能驗證才能標記 task/feature 完成——而不是「code 寫完了」就算完成？ | 硬閘門：E2E 測試通過才能標完成 = A, 軟提醒 = C, 寫完 code = 完成 = F |
| **反壓設計** | 系統有沒有同時施加上游反壓（引導 AI 走向正確實作）和下游反壓（阻擋不正確的工作）？ | 上游（現有模式/型別系統/架構約束）+ 下游（測試/lint/CI/安全掃描）= A, 只有下游 = B, 兩者都沒有 = F |
...
| **狀態格式韌性** | Feature/任務狀態是用機器可解析的格式儲存，還是 AI 可能意外覆蓋的自由格式？ | 結構化機器可解析格式（JSON/YAML + schema）= A, 半結構化 Markdown = C, 自由文字 = F |
...
| **多 Agent 設計** | 多個 AI agent 之間能不能協調，且每個 agent 的工具權限有沒有 scope 到它的角色？ | 協調協議 + 衝突解決 + 角色化工具權限 = A, 有協調但共享權限 = B, 沒考慮 = F |
| **Agent 可觀測性存取** | AI agent 能不能查詢 log、metrics、runtime 狀態、DOM 來自我診斷問題——而不是只能等人類介入？ | agent 能存取 logs/metrics/spans + 瀏覽器自動化工具 = A, 部分存取 = C, agent 完全看不到系統內部 = F |
...
這個框架評估的是 **harness 設計品質**——結構性和執行性屬性。它有已知的盲點：

**1. 不評估功能正確性。**
這個 eval 能告訴你系統是否*強制執行*完成閘門，是否*要求*驗證。但它無法告訴你 agent 的輸出在功能上是否正確。架構約束、linting、CI 全部通過，feature 仍然可能是壞的。E2E 驗證閘門指標只是偵測「閘門是否存在」，不是「測試是否充分」。

**2. 這個 eval 假設的是綠地（greenfield）或受控環境。**
所有主要公開成功案例（OpenAI、Anthropic、Stripe、Hashimoto）都涉及新專案或全新環境。把 harness engineering 應用到有十年歷史、模式不一致、沒有架構測試的遺留 codebase 上，是完全不同的問題。把這個 eval 套到棕地（brownfield）專案上，分數會偏低——棕地系統需要的是*遷移路徑*，不只是設計評估。

**3. AI 生成 code 的長期可維護性沒有被評分。**
AI 生成的 code 六到十二個月後是否仍然可維護是一個開放問題（Brockman, 2025）。熵防禦指標評估的是機制，不是結果。有 GC agent = A；那個 GC agent 是否真的維持了 codebase 的可維護性，尚未驗證。

**4. 假設壓力測試（步驟 1.7）應該在每次重大 model 升級後重跑。**
現在需要的 harness 元件，在下一代 model 出來後可能變成 stale scaffolding。這個 eval 的評分在評估當下有效；model 能力提升可能讓高分元件變得冗餘。
